...
######[jayden]返回一个字典，包含当前时间的信息，如果当前时间为2015年12月10日13点19分13秒，则字典内容为：
############## {"year":"2015", "month":"12", "day":"10", "time":"131913", "datetime":"20151210131913", "rnd":"xxx"}
...
def UploadFile(request):
    ...
    #取得上传的文件的原始名称   #[jayden] 如果上传的文件名为file.txt, 则upload_original_name为file, upload_original_ext为txt
...
#######################################修改#################################################################
        if 'SERVER_SOFTWARE' in os.environ:
            if action=="uploadscrawl":
                state, url_temp=save_scrawl_file(request,os.path.join(OutputPath,OutputFile))
            else:
                #保存到文件中，如果保存错误，需要返回ERROR
                state, url_temp=save_upload_file(file,os.path.join(OutputPath,OutputFile))
        else:
...
def get_output_path(request,path_format,path_format_var):
    #取得输出文件的路径
#############[jayden] 如果在模型中设置了imagePath="blog/article_image/", filePath="blog/article_file/"。
#############则OutputPathFormat为blog/article_image/%(basename)s_%(datetime)s_%(rnd)s.%(extname)s
    OutputPathFormat=(request.GET.get(path_format,USettings.UEditorSettings["defaultPathFormat"]) % path_format_var).replace("\\","/")
    #分解OutputPathFormat
#############[jayden] OutputPath为模型中设置的上传路径，即blog/article_image/。
#############OutputFile为文件名%(basename)s_%(datetime)s_%(rnd)s.%(extname)s
...
#########################结束##################################################
        ...
        try:
            content=request.POST.get(USettings.UEditorUploadSettings.get("scrawlFieldName","upfile"))
            f = open(filename, 'wb')
            f.write(base64.decodestring(str.encode(content)))
            f.close()
            state="SUCCESS"
        except Exception as E:
            # errorno, errorstr = E.args
            state="写入图片文件错误:\n{0}".format(E.args)
        return state
